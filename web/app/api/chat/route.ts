...
import { geminiGenerate, GeminiNotConfigured, type GeminiMessage } from "@/lib/gemini";
...
export async function POST(req: NextRequest) {
  ...
  const messages: GeminiMessage[] = raw
    .slice(-12)
    .map((m: { role?: string; content?: string }) => ({
      role: m.role === "assistant" ? "model" : "user",
      text: String(m.content || ""),
    }))
    .filter((m: GeminiMessage) => m.text.trim().length > 0);
  ...
  try {
    const reply = await geminiGenerate({ system: SYSTEM, messages, temperature: 0.7, maxTokens: 400 });
    return NextResponse.json({ reply });
  } catch (e) {
    if (e instanceof GeminiNotConfigured) {
      return NextResponse.json({
        reply:
          "I'm not switched on yet — add a GEMINI_API_KEY in web/.env.local to enable the AI assistant. 🙂",
      });
    }
    return NextResponse.json({
